...
First we sue TMLE on the motivating example 20.1 from Ch. 20, without and with an additional L1 effect, added by us. 
...
fit_wrong <- kang_schafer %>% lm(y ~ t + x1 + x2 + x3 + x4, data = .)  
fit_correct <- kang_schafer %>% lm(y ~ t + z1 + z2 + z3 + z4, data = .)  
summary(fit_wrong)
